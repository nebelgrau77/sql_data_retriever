...
fn input_step(app: &mut Cursive) {
    ...
    app.add_layer(
        Dialog::new()
            ...
            .button("submit", |s| {
                let key_a = s
                    .call_on_name("season", |t: &mut EditView| t.get_content())
                    .unwrap();

                let key_a: u8 = key_a.trim().parse().expect("something didn't work");

                let key_b = s
                    .call_on_name("episode", |t: &mut EditView| t.get_content())
                    .unwrap();
...
fn get_title(keys: &Keys) -> Result<String> {
    ...
    let query = format!(
        "SELECT title FROM bosch WHERE season = {} AND episode = {}",
        keys.ses_num, keys.ep_num
    );
